# Shell Article

## Syntax

- [GitHub - bahamas10/bash-style-guide: A style guide for writing safe, predictable, and portable bash scripts (not sh!)](https://github.com/bahamas10/bash-style-guide)
- [Shell Script Suggestions for Speedy Setups](https://thoughtbot.com/blog/shell-script-suggestions-for-speedy-setups)
- [The Unix Shell's Humble If](https://thoughtbot.com/blog/the-unix-shells-humble-if#syntax)
- [Input/Output Redirection in the Shell](https://thoughtbot.com/blog/input-output-redirection-in-the-shell)
- [Shell 扩展-参数扩展(Shell Parameter Expansion) - 知乎](https://zhuanlan.zhihu.com/p/65599187)

## Tools

- [Linux - rm -rf 刪除文件指定移動文件位置 （李作強） - 台部落](https://www.twblogs.net/a/5c0a21c0bd9eee6fb37b6cb1)

## Website

- [nixCraft - Linux Tips, Hacks, Tutorials, And Ideas In Blog](https://www.cyberciti.biz/)
- [Shell 設計入門](http://mirror.sars.tw/Bash_Shell_by_ols3/book1.html)

## Note

- [Shell Command 筆記 - iT 邦幫忙::一起幫忙解決難題，拯救 IT 人的一天](https://ithelp.ithome.com.tw/articles/10189741)
